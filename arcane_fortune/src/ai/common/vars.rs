...
pub struct CityState<'bt,'ut,'rt,'dt> {
	...
	// if more entries added here:
		pub population_center_ind: Option<usize>, // city hall/manor once it's created
...
impl_saving!{CityState<'bt,'ut,'rt,'dt> {coord, gate_loc, wall_coords,
	...
	population_center_ind, boot_camp_ind,
...
impl CityState<'_,'_,'_,'_> {
	...
	pub fn register_bldg(&mut self, bldg_ind: usize, b: &Bldg) {
		if let BldgType::Taxable(_) = b.template.bldg_type {return;}
		
		match b.template.nm[0].as_str() {
			BOOT_CAMP_NM => {
	...
	pub fn chg_bldg_ind(&mut self, frm_ind: usize, to_ind: usize, b: &Bldg) -> bool {
		if let BldgType::Taxable(_) = b.template.bldg_type {return false;}
		
		if let BldgArgs::PopulationCenter {..} = &b.args {
			if self.population_center_ind == Some(frm_ind) {
				self.population_center_ind = Some(to_ind);
				return true;
			}
		}
...
impl <'bt,'ut,'rt,'dt>AIState<'bt,'ut,'rt,'dt> {
	pub fn add_bldg(&mut self, bldg_ind: usize, bldg_coord: u64, b: &Bldg, map_sz: MapSz) {
		if let BldgType::Taxable(_) = b.template.bldg_type {return;}
		...
		/*#[cfg(any(feature="opt_debug", debug_assertions))]
		{
			for city_state in self.city_states.iter() {
				if bldg_template.nm[0] == CITY_HALL_NM {
					debug_assertq!(city_state.population_center_ind != Some(bldg_ind), "bldg {} already added", bldg_ind);
		...
		/*{
			for city in self.city_states.iter() {
				printlnq!("city coord {} dist {} population_center_ind none {}", city.coord, manhattan_dist_inds(city.coord, bldg_coord, map_sz),
						city.population_center_ind.is_none());
			}
	...
	pub fn rm_bldg(&mut self, bldg_ind: usize, b: &Bldg, disband_unit_inds: &mut Vec<usize>) {
		...
		if let BldgArgs::PopulationCenter {..} = &b.args {
			for (city_ind, city_state) in self.city_states.iter_mut().enumerate() {
				...
				if city_state.population_center_ind == Some(bldg_ind) {
					// rm units
	...
	pub fn chg_bldg_ind(&mut self, frm_ind: usize, to_ind: usize, b: &Bldg) {
		if let BldgType::Taxable(_) = b.template.bldg_type {return;}
		
		self.city_states.iter_mut().any(|cs| cs.chg_bldg_ind(frm_ind, to_ind, b));
	}
...
pub fn transfer_city_ai_state<'bt,'ut,'rt,'dt>(population_center_ind: usize, frm_owner_id: usize, to_owner_id: usize, cur_player: usize,
	...
	if let Some(frm_ai_state) = player_frm.ptype.any_ai_state_mut() {
		if let Some(city_state_ind) = frm_ai_state.city_states.iter().position(|cs| cs.population_center_ind == Some(population_center_ind)) {
			...
			if let Some(_to_ai_state) = players[to_owner_id].ptype.any_ai_state() {
			...
			for (bldg_ind, b) in bldgs.iter().enumerate() {
				if b.owner_id == to_owner_id as SmSvType {
					if let BldgArgs::PopulationCenter {..} = b.args {
						printlnq!("population center coord {} {}", Coord::frm_ind(b.coord, map_sz), bldg_ind);
					}
				}
			}
	...
	// should no longer occur because constructing a city hall should create an entry in the ai_state --- add new entry to receiver (sender is human)
		...
		let b = &bldgs[population_center_ind];
		...
		return Some(CityState {
				...
				population_center_ind: Some(population_center_ind),
				boot_camp_ind: None,
				academy_ind: None,
