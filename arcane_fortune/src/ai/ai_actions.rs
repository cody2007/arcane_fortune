use super::*;
use crate::map::{MapSz, Owner, PlayerType, ZoomInd, Stats};
use crate::units::*;
use crate::disp::{Coord, ActionInterfaceMeta};
use crate::movement::{MvVars, MvVarsAtZoom, Dest, movable_to, find_square_buildable};
...
impl <'bt,'ut,'rt,'dt>AIState<'bt,'ut,'rt,'dt> {
	pub fn plan_actions(&mut self, ai_ind: usize, owner: &Owner, units: &mut Vec<Unit<'bt,'ut,'rt,'dt>>, bldgs: &mut Vec<Bldg<'bt,'ut,'rt,'dt>>,
			stats: &mut Vec<Stats<'bt,'ut,'rt,'dt>>, relations: &mut Relations, map_data: &mut MapData<'rt>, exs: &mut Vec<HashedMapEx<'bt,'ut,'rt,'dt>>, owners: &Vec<Owner>,
			unit_templates: &'ut Vec<UnitTemplate<'rt>>, bldg_templates: &'bt Vec<BldgTemplate<'ut,'rt,'dt>>,
			disband_unit_inds: &mut Vec<usize>,
			logs: &mut Vec<Log>, rng: &mut XorState, map_sz: MapSz, turn: usize, iface_settings: &mut IfaceSettings, ai_config: &AIConfig,
		...
		let pstats = &stats[owner.id as usize];
		if !pstats.alive {return;}
		
		if let PlayerType::AI(personality) = &owner.player_type {
			...
			// war planning (declaration)
			{
				...
				let icbm_producable = pstats.unit_producable(UnitTemplate::frm_str(ICBM_NM, unit_templates));
				
				if rng.gen_f32b() <= WAR_CHECK_PROB*(1. - personality.friendliness) {
					let war_enemies = relations.at_war_with(owner.id as usize);
					
					// potential targets
					'attackee_loop: for ((owner_ind, p_owner), pstats) in owners.iter().enumerate().zip(stats.iter()) {
						if owner_ind == owner.id as usize {continue;} // don't launch war against self
						if war_enemies.contains(&owner_ind) {continue;} // already at war
						if p_owner.player_type == PlayerType::Barbarian {continue;}
						
						// civ and the human player have not yet met:
						if p_owner.player_type == PlayerType::Human && !relations.discovered(owner_ind as usize, owner.id as usize) {continue;}
						
						if let Some(adv) = self.current_war_advantage(owner.id as usize, stats, relations) {
							if adv <= *pstats.defense_power_log.last().unwrap() as isize {continue;} // cannot overpower target with current wars
							...
							for city in self.city_states.iter() {
								...
								for b in bldgs.iter() {
									if b.owner_id as usize != owner_ind {continue;}
									if !icbm_producable && MAX_WAR_DIST <= manhattan_dist(Coord::frm_ind(b.coord, map_sz), city_loc, map_sz) {continue;}
									
									relations.declare_war(owner.id as usize, owner_ind, logs, owners, turn, iface_settings, cur_ui_ai_player_is_paused, disp_settings, menu_options, rng, d);
					...
					if let Some(&worker_ind) = city_parent.worker_inds.last() {
						match city_parent.neighbors_possible {
							Neighbors::Possible | Neighbors::NotKnown => {
								...
								if let Some(coord) = city_parent.find_new_city_loc(self, pstats, map_data, exs.last().unwrap(), ai_config, rng, map_sz) {
									let city_parent = &mut self.city_states[rand_city_ind];
									
									city_parent.neighbors_possible = Neighbors::Possible;
									city_parent.worker_inds.pop(); // remove worker_ind from old city
									
									self.create_city_plan(coord, rng, map_data, map_sz, bldg_templates);
									self.city_states.last_mut().unwrap().worker_inds.push(worker_ind); // add worker_ind to new city
									
									//dbg_log("creating new city", owner.id, logs, turn);
								}else{
									self.city_states[rand_city_ind].neighbors_possible = Neighbors::NotPossible;
								}
			...
			{
				let boot_camp_template = &BldgTemplate::frm_str(BOOT_CAMP_NM, bldg_templates);
				let academy_template = &BldgTemplate::frm_str(ACADEMY_NM, bldg_templates);
				...
				for city_ind in 0..self.city_states.len() {
					...
					city.execute_defense_actions(is_cur_player, units, owner, bldgs, owners, pstats, relations, map_data, exs, disband_unit_inds, map_sz, turn, logs);
					if rng.gen_f32b() < 0.25 {
						city.execute_worker_actions(is_cur_player, units, owner, bldgs, map_data, exs, pstats, relations, owners, logs, map_sz, turn);
					}
					...
					/////////// worker actions
					{
						...
						macro_rules! add_bldg_action{($bldg_template: expr) => {
							...
							if !already_planned {
								...
								if !already_planned {
									...
									search_start.y += rng.isize_range(-CH_DIST, CH_DIST);
									search_start.x += rng.isize_range(-CH_DIST, CH_DIST);
						...
						if city.ch_ind != None {
							...
							// bonus bldg
								...
								self.set_next_bonus_bldg(pstats, &personality, bldg_templates, rng);
			...
			// building productions: (produce new workers, defense units, attack units)
			{
				...
				let max_defensive_unit = pstats.max_defensive_unit(unit_templates);
				let net_income = pstats.net_income();
				
				for city in self.city_states.iter() {
					// city hall
					if let Some(ch_ind) = city.ch_ind {
						if let BldgArgs::CityHall {ref mut production, ..} = &mut bldgs[ch_ind].args {
							...
							if production.len() == 0 && city.worker_inds.len() < WORKERS_PER_CITY {
								...
								production.push(ProductionEntry {
									production: UnitTemplate::frm_str(WORKER_NM, unit_templates),
									progress: 0
								});
							}
						}else{panicq!("ai city hall has no cityhall arguments");}
					}
						...
						if let Some(boot_camp_ind) = city.boot_camp_ind {
							if let BldgArgs::GenericProducable {ref mut production} = &mut bldgs[boot_camp_ind].args {
								...
								if production.len() == 0 {
									...
									}else if let Some(next_attack_unit) = self.attack_fronts.next_req_unit(self.city_states.len(), pstats, unit_templates) {
			...
			self.attack_fronts.execute_actions(ai_ind, units, bldgs, map_data, exs, relations, map_sz, rng, logs);//, iface_settings);
			
			//////////////////////////////
			// icbm actions
			if self.icbm_inds.len() > 0 && rng.gen_f32b() < (1./10.){
				let war_enemies = relations.at_war_with(owner.id as usize);
		...
		}else{panicq!("invalid owner setting");}
	} // plan unit actions
	
	pub fn accept_peace_treaty(&self, opposition_id: usize, cur_player_id: usize,
			gold_offering: f32, relations: &mut Relations, stats: &Vec<Stats>,
			owners: &Vec<Owner>, turn: usize) -> bool {
		...
		let friendliness = relations.friendliness_toward(opposition_id, cur_player_id, owners);
		
		if let Some(war_advantage) = self.current_war_advantage(opposition_id, stats, relations) {
...
impl <'bt,'ut,'rt,'dt>CityState<'bt,'ut,'rt,'dt> {
	/*pub fn execute_attack_actions(&mut self, war_enemies: &Vec<usize>, units: &mut Vec<Unit<'bt,'ut,'rt,'dt>>, owner: &Owner, bldgs: &Vec<Bldg<'bt,'ut,'rt,'dt>>, map_data: &mut MapData,
			exs: &mut Vec<HashedMapEx<'bt,'ut,'rt,'dt>>, owners: &Vec<Owner>, map_sz: MapSz, turn: usize, relations: &mut Relations, logs: &mut Vec<Log>) {
		#[cfg(feature="profile")]
		let _g = Guard::new("execute_attack_actions");
		
		// not currently at war, nothing to do
		if war_enemies.len() == 0 {return;}
		
		let mut prev_target = None;
		
		'attacker_loop: for unit_ind in self.attacker_inds.iter() {
			let u = &mut units[*unit_ind];
			debug_assertq!(u.owner_id == owner.id && u.template.nm != WORKER_NM);
			...
			let chk_alter = || {
				...
				if let Some(ActionMeta {action_type: ActionType::Fortify {turn: turn_fortified}, ..}) = u.action.last() {
					if (*turn_fortified + TURNS_RECHECK_FORTIFY) < turn {return true;}
				}
				
				// unit is pressumably attacking, continue loop
				false
			};
			...
			if let Some(target) = &prev_target {
				if set_target_attackable(target, *unit_ind, Some(self), owner, units, bldgs, exs, map_data, owners, relations, logs, map_sz, turn) {
					continue 'attacker_loop;
				}
			}
			
			if let Some(new_target) = find_and_set_attack(*unit_ind, TargetOwner::FromVec(war_enemies), units, Some(self), bldgs, exs, owner, map_data, owners, relations, logs, map_sz, turn) {
				prev_target = Some(new_target);
			// no new target, update as failed attempt
			}else{
				units[*unit_ind].action = vec!{ActionMeta::new(ActionType::Fortify {turn}); 1};
			}
		}
	}*/
	
	pub fn execute_defense_actions(&mut self, is_cur_player: bool, units: &mut Vec<Unit<'bt,'ut,'rt,'dt>>, owner: &Owner, bldgs: &Vec<Bldg<'bt,'ut,'rt,'dt>>,
			owners: &Vec<Owner>, pstats: &mut Stats<'bt,'ut,'rt,'dt>, relations: &mut Relations, map_data: &mut MapData,
			exs: &mut Vec<HashedMapEx<'bt,'ut,'rt,'dt>>, disband_unit_inds: &mut Vec<usize>,
			map_sz: MapSz, turn: usize, logs: &mut Vec<Log>) {
		#[cfg(feature="profile")]
		...
		if self.defenders.len() > 0 {
			...
			'unowned_unit_loop: for unowned_unit_ind in 0..units.len() {
				...
				if unowned_unit.owner_id == owner.id {continue;}
				
				let unowned_owner = unowned_unit.owner_id as usize;
				
				// at war w/ and close enough to attack
				if (owners[unowned_owner].player_type == PlayerType::Barbarian || war_enemies.contains(&unowned_owner)) &&
						self.within_city_defense_area(Coord::frm_ind(unowned_unit.return_coord(), map_sz)) {
						...
						for (list_ind, defender) in self.defenders.iter().enumerate() {
							if let Some(action) = &units[defender.unit_ind].action.last() {
								if let ActionType::Fortify {turn: turn_fortified} = &action.action_type {
									if (*turn_fortified + TURNS_RECHECK_FORTIFY) < turn {
										ordered_defenders.push(list_ind);
									}
								}
							}
						...
						match u.template.movement_type {
							MovementType::Land => {
								...
								if !self.exit_city(&mut max_search_depth, defender_unit_ind, is_cur_player, false,
									...
									owner, map_data, exs, units, bldgs, pstats, relations, owners, logs, map_sz, turn) {
		...
		// if no need to attack, move to defense positions
		{
			...
			for defender_list_ind in 0..self.defenders.len() {
				...
				debug_assertq!(u.owner_id == owner.id && u.template.nm[0] != WORKER_NM);
				...
				let chk_alter = || {
					...
					// if fortified, only check so often
					if let Some(ActionMeta {action_type: ActionType::Fortify {turn: turn_fortified}, ..}) = u.action.last() {
						if (*turn_fortified + TURNS_RECHECK_FORTIFY) > turn {return false;}
					}
				...
				u.action.clear();
				u.action.push(ActionMeta::new(ActionType::Fortify {turn}));
				...
				if let Some(next_defense_pos_ind) = next_defense_pos_ind_opt {
					...
					action_iface.update_move_search(Coord::frm_ind(self.defense_positions[next_defense_pos_ind], map_sz), map_data, exs, MvVars::NonCivil{units, start_owner: owner.id, blind_undiscov: None}, bldgs);
					...
					if action_iface.action.path_coords.len() > 0 {
						...
						if defender.failed_mv_attempts > MAX_FAILED_MV_ATTEMPTS {
							...
							units[defender.unit_ind].action.push(ActionMeta::new(ActionType::Fortify {turn}));
						}
					}
				}
			}
	...
	pub fn execute_worker_actions(&mut self, is_cur_player: bool, units: &mut Vec<Unit<'bt,'ut,'rt,'dt>>, owner: &Owner, bldgs: &Vec<Bldg<'bt,'ut,'rt,'dt>>, map_data: &mut MapData,
			exs: &mut Vec<HashedMapEx<'bt,'ut,'rt,'dt>>, pstats: &mut Stats<'bt,'ut,'rt,'dt>, relations: &mut Relations,
			owners: &Vec<Owner>, logs: &mut Vec<Log>, map_sz: MapSz, turn: usize) {
		...
		if let Some(&unit_ind) = self.worker_inds.iter().find(|&&unit_ind| {
			...
			self.set_worker_action(unit_ind, is_cur_player, units, owner, bldgs, map_data, exs, pstats, relations, owners, logs, map_sz, turn);		
		} // idle worker exists
	}
	
	pub fn set_worker_action(&mut self, unit_ind: usize, is_cur_player: bool, units: &mut Vec<Unit<'bt,'ut,'rt,'dt>>, owner: &Owner, bldgs: &Vec<Bldg<'bt,'ut,'rt,'dt>>, map_data: &mut MapData,
			exs: &mut Vec<HashedMapEx<'bt,'ut,'rt,'dt>>, pstats: &mut Stats<'bt,'ut,'rt,'dt>, relations: &mut Relations,
			owners: &Vec<Owner>, logs: &mut Vec<Log>, map_sz: MapSz, turn: usize) {
		if let Some(action) = self.worker_actions.pop() {
			action.set_action_meta(unit_ind, is_cur_player, units, owner, bldgs, map_data, Some(self), exs, pstats, relations, owners, logs, map_sz, turn);
		}
	}
	...
	pub fn exit_city(&self, max_search_depth: &mut usize, unit_ind: usize, is_cur_player: bool, min_dist_chk: bool, 
			start_coord: Coord, u_coord: Coord, owner: &Owner, map_data: &mut MapData, exs: &mut Vec<HashedMapEx<'bt,'ut,'rt,'dt>>, 
			units: &mut Vec<Unit<'bt,'ut,'rt,'dt>>, bldgs: &Vec<Bldg>, pstats: &mut Stats<'bt,'ut,'rt,'dt>, relations: &mut Relations, owners: &Vec<Owner>,
			logs: &mut Vec<Log>, map_sz: MapSz, turn: usize) -> bool {
		#[cfg(feature="profile")]
		...
		*max_search_depth *= 2*5; // was 2
		...
		if let Some(min_coord_ind) = map_sz.coord_wrap(min_coord.y, min_coord.x) {
			...
			if movable_to(u_coord_ind, min_coord_ind, &map_data.get(ZoomInd::Full, min_coord_ind), exs.last().unwrap(), MvVarsAtZoom::NonCivil {units, start_owner: owner.id, blind_undiscov: None},
					bldgs, &Dest::NoAttack, units[unit_ind].template.movement_type) {
				set_coord(min_coord_ind, unit_ind, is_cur_player, units, map_data, exs, pstats, owners, map_sz, relations, logs, turn);
