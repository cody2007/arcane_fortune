...
impl BarbarianState {
	pub fn plan_actions<'bt,'ut,'rt,'dt>(player_ind: usize, units: &mut Vec<Unit<'bt,'ut,'rt,'dt>>,
			...
			players: &mut Vec<Player>, unit_templates: &'ut Vec<UnitTemplate<'rt>>,
			map_sz: MapSz, rng: &mut XorState, turn: usize) {
		if turn <= (GAME_START_TURN + 30*12*15) {return;}
		...
		'barbarian_attempt: loop {
			...
			macro_rules! try_again{() => {
				if *attempt > N_UNIT_PLACEMENT_ATTEMPTS { // try a new map
					...
					*attempt += 1;
					print_attempt_update(*attempt, player_ind, &sz_prog, screen_sz, disp_chars, l, d);
