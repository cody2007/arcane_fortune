...
fn print_tree_recur<T: TreeTemplate>(mut row: i32, col: i32, root_children: &Vec<SmSvType>,
		...
		parent_tech: Option<SmSvType>, pstats: &Stats,
		entry_sel: Option<SmSvType>, row_start: i32, row_stop: i32, col_stop: i32, entry_sz_print: ScreenSz,
		sel_loc: &mut Option<(i32, i32, SmSvType)>, dstate: &mut DispState) {
	...
	for t in root_children.iter() {
		...
		strs_show.push(tp.nm(&dstate.local));
		strs_show.push(tp.requirements_txt(pstats, &dstate.local));
		
		// requires multiple techs
		if let Some(techs_req) = tp.reqs() {
			if techs_req.len() != 1 {
				strs_show.push(dstate.local.Also_req.clone());
				...
				for (i, tech_req) in techs_req.iter().enumerate() {
					if *tech_req != parent_tech.unwrap() {
						...
						if (i+1) == techs_req.len() || techs_req[i+1] == parent_tech.unwrap() {
							strs_show[str_added].push_str(&format!("{}", templates[*tech_req as usize].nm(&dstate.local)));
						}else{
							strs_show[str_added].push_str(&format!("{}, ", templates[*tech_req as usize].nm(&dstate.local)));
						}
					}
		...
		print_tree_recur(row, col + entry_sz_print.w as i32, &sub_children, templates, cur_depth+1, 
				shown, Some(*t), pstats, entry_sel, row_start, row_stop, col_stop,
				entry_sz_print, sel_loc, dstate);
...
impl <'f,'bt,'ut,'rt,'dt>DispState<'f,'_,'bt,'ut,'rt,'dt> {
	pub fn print_tree<T: TreeTemplate>(&mut self, templates: &Vec<T>, pstats: &Stats, 
		...
		print_tree_recur(tree_offsets.row, tree_offsets.col, &root_children, 
				...
				pstats, *tech_sel, row_tree_start, row_tree_stop, col_tree_stop,
				entry_sz_print, &mut sel_loc, self);
		...
		// esc to close
		d.mv(0,0); self.buttons.Esc_to_close.print(None, &self.local, d);
