macro_rules! do_txt_entry_keys{($key_pressed: expr, $curs_col: expr, $input_txt: expr, $printable_type: expr, $disp: expr) => {
	match $key_pressed {
		KEY_ESC => {panicq!("todo: handle esc");}//$disp.end_window();}
		k if k == '\n' as i32 => {
		...
		_ => { // insert character
			if $input_txt.len() < (min(MAX_SAVE_AS_W, $disp.iface_settings.screen_sz.w)-5) {
