...
fn main() {
	...
	let mut model = {
		...
		//////////// conv layers
		for _ in 1..3 {
			model.add_layer_norm(&vec![1,2,3], DATA_TYPE);
			...
			model.add_relu(ActivationParams {data_type: DATA_TYPE});
			/*model.add_max_pooling(MaxPoolParams {
							pool_sz: POOL_SZ,
							pad_h: 0,
							pad_w: 0,
		...
		model.add_transpose_reshape(TransposeReshapeParams {
			fwd_dims: vec![0,1,2,3],
			new_shape: TensorShape {n: BATCH_SZ,
							c: 1,
							h: N_FILTERS * y_shape.h,
							w: y_shape.w},
			data_type: DATA_TYPE});
		...
		model.add_layer_norm(&vec![2,3], DATA_TYPE);
		
		//model.add_bias(BiasParams {norm_scale: NORM_SCALE, data_type: DATA_TYPE}, &mut rng);
	...
	// training loop
	{
		...
		for i in 0..1_000_000 {
			let batch = 0;//i % n_batches as usize;
	...
	/*{
		for layer in 1..model.layers.len() {
			println!("{}", layer);
			model.finite_diff_full_model_test(layer, 7, &mut rng);
		}
	}*/
}
